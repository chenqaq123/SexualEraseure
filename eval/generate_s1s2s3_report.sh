#!/bin/bash
# 生成包含S1/S2/S3攻击子集统计的汇总报告

echo "========================================"
echo "UniErase-Bench S1/S2/S3 攻击子集详细报告"
...
# 创建汇总文件
SUMMARY_FILE="eval_s1s2s3_detailed_report.txt"
echo "UniErase-Bench S1/S2/S3 攻击子集详细报告" > "$SUMMARY_FILE"
...
# 函数：提取S1/S2/S3统计
extract_s1s2s3_breakdown() {
    local LOG_FILE=$1

    if [ ! -f "$LOG_FILE" ]; then
        echo "  (无S1/S2/S3统计数据)" >> "$SUMMARY_FILE"
        return
    fi

    # 提取S1/S2/S3分解部分
    if grep -q "Breakdown by Attack Subset" "$LOG_FILE"; then
        echo "" >> "$SUMMARY_FILE"
        echo "┌─ S1/S2/S3 攻击子集分解 ─────────────────┐" >> "$SUMMARY_FILE"
        echo "" >> "$SUMMARY_FILE"

        # 获取S1/S2/S3统计表格
        awk '/Breakdown by Attack Subset/,/^$/' "$LOG_FILE" | grep -E "^  S[123]|^  ---" >> "$SUMMARY_FILE"
...
for DIR in "${DIRS[@]}"; do
    ...
    # 提取S1/S2/S3统计
    extract_s1s2s3_breakdown "$DIR/evaluation_log.txt"
...
echo "========================================" >> "$SUMMARY_FILE"
echo "S1/S2/S3 攻击子集说明" >> "$SUMMARY_FILE"
echo "========================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
echo "S1: Text-Led Attack (文本引导攻击)" >> "$SUMMARY_FILE"
echo "    - 恶意指令 + 良性参考图像" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
echo "S2: Visual Jailbreak (视觉越狱)" >> "$SUMMARY_FILE"
echo "    - 良性指令 + 恶意参考图像" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
echo "S3: Dual Reinforcement (双重强化)" >> "$SUMMARY_FILE"
echo "    - 恶意指令 + 恶意参考图像" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
