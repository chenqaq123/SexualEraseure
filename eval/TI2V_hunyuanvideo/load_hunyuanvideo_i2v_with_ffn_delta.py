"""
Load HunyuanVideo-1.5 I2V, apply multi-layer FFN delta patch,
run UniErase-Bench TI2V.
...
BENCHMARK_PATH = SCRIPT_DIR / "../../prompt_data/UniErase-Bench/ti2v_benchmark.jsonl"
BENCHMARK_IMG_DIR = (
    SCRIPT_DIR / "../../prompt_data/UniErase-Bench/reference_images/ti2v_benchmark"
)
...
REFERENCE_IMAGE_REMAP = {
    "/home/mlsnrs/data/cgx/cgx_nas/T2I/Concept_Erasure/prompt_data/UniErase-Bench":
        str((SCRIPT_DIR / "../../prompt_data/UniErase-Bench").resolve()),
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Load HunyuanVideo I2V pipeline, apply a saved multi-layer FFN delta "
            "patch, and evaluate on UniErase-Bench TI2V."
        )
    )
...
def remap_image_path(path: str) -> str:
    ...
    # Fall back to using just the filename in the local benchmark image dir
    fname = Path(path).name
    local = BENCHMARK_IMG_DIR / fname
    if local.exists():
        return str(local)
    return path
...
def run_benchmark(pipe, output_dir: Path, benchmark_path: Path):
    ...
    try:
        for idx, item in enumerate(benchmark_data):
            ...
            out_path = output_dir / f"idx{idx:04d}_sid{source_id}.mp4"
            try:
