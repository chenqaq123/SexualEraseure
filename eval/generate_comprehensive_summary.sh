...
# 综合汇总报告生成脚本
# 包含 TI2I、T2V、TI2V 三种类型的完整评估数据汇总

BASE_DIR="/home/mlsnrs/data/cgx/cgx_nas/T2I/Concept_Erasure/SexualErasure_CCS/eval"
OUTPUT_FILE="$BASE_DIR/comprehensive_evaluation_summary.txt"

echo "========================================" > "$OUTPUT_FILE"
echo "综合评估汇总报告" >> "$OUTPUT_FILE"
echo "包含: TI2I、T2V、TI2V 三种类型" >> "$OUTPUT_FILE"
...
if "TI2I" in eval_type:
    ...
    print("| 子集 | 平均TC  | 平均Motion Score | 平均DOVER |")
    print("|------|---------|------------------|-----------|")
    print(f"| S1   | {s1_stats['avg_tc']:7.4f} | {s1_stats['avg_motion']:16.4f} | {s1_stats['avg_dover']:9.4f} |")
    print(f"| S2   | {s2_stats['avg_tc']:7.4f} | {s2_stats['avg_motion']:16.4f} | {s2_stats['avg_dover']:9.4f} |")
    print(f"| S3   | {s3_stats['avg_tc']:7.4f} | {s3_stats['avg_motion']:16.4f} | {s3_stats['avg_dover']:9.4f} |")
    print(f"| 总体 | {total_stats['avg_tc']:7.4f} | {total_stats['avg_motion']:16.4f} | {total_stats['avg_dover']:9.4f} |")
print()
...
echo "" >> "$OUTPUT_FILE"
echo "========================================" >> "$OUTPUT_FILE"
echo "五、S1/S2/S3 攻击子集统一说明" >> "$OUTPUT_FILE"
echo "========================================" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
cat << 'EOF' >> "$OUTPUT_FILE"
