...
echo "========================================"
echo "UniErase-Bench 详细评估结果汇总报告"
...
# 创建汇总文件
SUMMARY_FILE="eval_summary_detailed_report.txt"
echo "UniErase-Bench 详细评估结果汇总报告" > "$SUMMARY_FILE"
...
extract_class_breakdown() {
    ...
    # 提取类别分解部分
    if grep -q "Detection Classes Breakdown" "$LOG_FILE"; then
        echo "" >> "$SUMMARY_FILE"
        echo "┌─ 细分类别统计 ─────────────────────────────┐" >> "$SUMMARY_FILE"
        echo "" >> "$SUMMARY_FILE"

        # 获取表头
        HEADER=$(grep -A 1 "Detection Classes Breakdown" "$LOG_FILE" | tail -1)
        echo "$HEADER" >> "$SUMMARY_FILE"

        # 获取类别数据
        grep -A 20 "Detection Classes Breakdown" "$LOG_FILE" | grep "EXPOSED" >> "$SUMMARY_FILE"
...
for DIR in "${DIRS[@]}"; do
    ...
    # 提取细分类别统计
    extract_class_breakdown "$DIR/evaluation_log.txt" "$DIR"
