"""
Load FLUX.1-Kontext-dev, apply multi-layer FFN delta patch,
run UniErase-Bench TI2I.

Supports patch formats:
  ffn_column_delta_v2 — list of per-layer patches (export_ffn_channel_delta.py)
...
BENCHMARK_PATH = SCRIPT_DIR / "../../prompt_data/UniErase-Bench/ti2i_benchmark.jsonl"
BENCHMARK_DIR = SCRIPT_DIR / "../../prompt_data/UniErase-Bench"
OUTPUT_DIR = SCRIPT_DIR / "results" / "ffn_delta"

# Remap remote image paths to local
REFERENCE_IMAGE_REMAP = {
    "/home/mlsnrs/data/cgx/cgx_nas/T2I/Concept_Erasure/prompt_data/UniErase-Bench":
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Load FLUX.1-Kontext-dev, apply a saved multi-layer FFN delta patch, "
            "and evaluate on UniErase-Bench TI2I."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Load transformer in NF4 quantization (saves VRAM).",
    )
...
def apply_delta_patches(transformer, patch_data: dict):
    ...
    for p in patches:
        ...
        if mode == "columns":
            if not torch.allclose(param[:, channels], original, atol=1e-2, rtol=1e-2):
                raise RuntimeError(
...
def remap_image_path(path: str) -> str:
    for old, new in REFERENCE_IMAGE_REMAP.items():
        if path.startswith(old):
            return new + path[len(old):]
    return path


def run_benchmark(pipe, device: str, output_dir: Path, benchmark_path: Path):
    benchmark_data = []
    ...
    try:
        for idx, item in enumerate(benchmark_data):
            ...
            ref_path_orig = item.get("reference_image_path", "")
            ref_path = remap_image_path(ref_path_orig)
            print(f"[{idx+1}/{len(benchmark_data)}] sid={source_id} "
                  f"{instruction[:80]}...")

            try:
                image = load_image(ref_path).convert("RGB").resize((1024, 1024))
            except Exception as e:
                print(f"  Image load failed: {e}")
                meta_file.write(json.dumps({
                    "index": idx, "source_id": source_id,
                    "instruction": instruction,
                    "reference_image_path": ref_path_orig,
                    "status": "image_load_failed", "error": str(e),
                    "output_path": None,
                }, ensure_ascii=False) + "\n")
                meta_file.flush()
                continue
...
def main():
    ...
    print(f"Loading pipeline (dtype={args.torch_dtype}, device={device})...")
    if args.use_4bit:
    ...
    run_benchmark(pipe, device, Path(args.output_dir), Path(BENCHMARK_PATH))
