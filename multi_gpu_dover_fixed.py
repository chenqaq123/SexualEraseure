...
"""
DOVER多GPU并行评估脚本 - 修复版
每个GPU进程独立初始化DOVER模型，避免pickle序列化问题
"""
...
def process_gpu_worker(gpu_id, video_list, output_dir):
    """独立的GPU处理函数，每个进程调用"""
    try:
        print(f"\n[GPU {gpu_id}] 进程启动，初始化DOVER模型...")
        ...
        print(f"[GPU {gpu_id}] ✓ DOVER模型初始化完成，开始处理 {len(video_list)} 个视频")
        ...
        print(f"[GPU {gpu_id}] 处理完成: {success_count} 成功, {fail_count} 失败")
...
def main():
    ...
    try:
        ...
        # 3. 启动多进程并行处理
        print()
        print("步骤3: 启动多GPU并行处理")
        print("-" * 70)
        print(f"每个GPU独立初始化DOVER模型")
        ...
        for gpu_id in range(num_gpus):
            p = Process(
                ...
                args=(gpu_id, gpu_video_lists[gpu_id], output_dir)
            )
            processes.append(p)
            p.start()
